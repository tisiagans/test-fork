...
Konfigurasi sistem pada makalah ini menunjukkan rancangan lingkungan kerja berbasis Arch Linux yang dirancang dengan fokus pada keamanan, efisiensi, fleksibilitas, dan performa sistem. Sistem ini menerapkan metode enkripsi LUKS on LVM guna memberikan perlindungan data sekaligus fleksibilitas pengelolaan partisi. Antarmuka desktop menggunakan XFCE karena ringan dan stabil, layout disk mengikuti standar CIS (Center for Internet Security), dan pengamanan jaringan menggunakan iptables sebagai firewall.

## B. Rumusan Masalah

- Bagaimana langkah instalasi Booster?
- Bagaimana langkah disk enkripsi LUKS on LVM?
- Bagaimana langkah menginstall desktop XFCE?
- Bagaimana langkah menginstall Superfile?
- Bagaimana langkah disk layout CIS?
- Bagaimana langkah instalasi Podman dan Podman Desktop?
- Bagaimana langkah instalasi Firewall iptables?
- Bagaimana langkah instalasi MPD, MPC, dan MPV?
- Bagaimana langkah instalasi password KeePassXC?
- Bagaimana langkah instalasi key secret?
- Bagaimana langkah akses OpenSSH?

## C. Tujuan

- Menjelaskan langkah instalasi Booster
- Menjelaskan langkah disk enkripsi LUKS on LVM
- Menjelaskan langkah menginstall desktop XFCE
- Menjelaskan langkah menginstall Superfile
- Menjelaskan langkah disk layout CIS
- Menjelaskan langkah instalasi Podman dan Podman Desktop
- Menjelaskan langkah instalasi Firewall iptables
- Menjelaskan langkah instalasi MPD, MPC, dan MPV
- Menjelaskan langkah instalasi password KeePassXC
- Menjelaskan langkah instalasi key secret
- Menjelaskan langkah penggunaan akses OpenSSH
...
## 23. Autostart Omeka-S dengan systemd User Service
...
**Penjelasan:**
- `~/.config/systemd/user/omeka.service` — File unit systemd di level user (tidak memerlukan hak root).
- `Description` — Deskripsi layanan yang muncul di `systemctl status`.
- `After=network.target` — Layanan ini dijalankan setelah jaringan tersedia.
- `WorkingDirectory=%h/containers/omeka` — `%h` adalah shortcut untuk direktori home user.
- `ExecStart=podman-compose up -d` — Perintah yang dijalankan saat service distart (menjalankan container Omeka).
- `ExecStop=podman-compose down` — Perintah yang dijalankan saat service dihentikan.
- `Type=oneshot` — Systemd menganggap service selesai setelah `ExecStart` selesai dieksekusi.
- `RemainAfterExit=yes` — Service dianggap masih "aktif" meski proses `ExecStart` sudah selesai (karena container berjalan di background).
- `WantedBy=default.target` — Service diaktifkan pada target default user session.
- `systemctl --user enable --now omeka` — Mengaktifkan dan langsung menjalankan service user Omeka.
- `loginctl enable-linger riski` — Mengizinkan service user milik `riski` tetap berjalan meski user belum login (penting agar Omeka tetap aktif saat reboot tanpa perlu login manual).
